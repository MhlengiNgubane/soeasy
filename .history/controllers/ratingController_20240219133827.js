...
module.exports = {
    addRating: async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({status: true, message: "Rating updated successfuly"});
        } catch (error) {
    ...
    checkUserRating: async (req, res) => {
        ...
        try {
            ...
            if(existingRating){
                res.status(200).json({status: true, message: "You have already submitted the rating"});
            } else{
                res.status(200).json({status: false, message: "User is yet to submit the rating"});
            }
        } catch (error) {
            res.status(500).json({status: false, message: error.message});
        }
